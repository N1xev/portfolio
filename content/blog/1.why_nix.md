...
So yeah Node 20 and Node 24 can coexist. Go 1.24 and 1.25 on the same system. Python 3.9 and 3.11 simultaneously. **No conflicts. Ever.** Each project gets exactly what it needs.
::
...
Instead of writing instructions for how to build an environment, you're **sharing the actual environment itself**. Like `package.json` + `package-lock.json` but for your **entire development setup**, not just JavaScript packages.
::
...
::callout{color="success" icon="i-lucide-shield-check"}
You literally can't accidentally break reproducibility. The system **enforces it mathematically**. It's not a convention or a best practice it's the only way the system works.
::
...
::note
Imperative is "do this, then do that." Declarative is "the system should look like this." One creates state. The other **describes** state.
::
...
- Steeper learning curve (not gonna sugarcoat it)
- Smaller ecosystem
...
Here's the cool part you can use Nix to build **reproducible Docker images**.
...
Same `flake.lock` = **identical environment** everywhere. Not "similar." Not "should work." **Identical.** Cryptographically guaranteed.
::
...
- First month: "I think I'm getting it... maybe"
- First six months: "Now I actually understand"

You'll hit walls. Get frustrated. Question if it's worth it. I definitely did. Spent an entire weekend debugging why my shell wouldn't source properly. Turned out I forgot a semicolon. **This is normal.**
::
